...
<template>
	<TransitionGroup tag="div" class="gz-modal-view" name="gz-modal">
		<div v-for="entry in router.modalStack" :key="entry.id" class="gz-modal-view__backdrop" :style="entry.matched.route.meta?.modalStyle" @click.self="onBackdropClick">
			<EntryProvider :entry-id="entry.id" :chain="entry.matched.chain" :params="entry.matched.params">
				<component :is="entry.matched.chain[0].component" v-bind="entry.matched.params" class="gz-modal-view__dialog" />
			</EntryProvider>
		</div>
	</TransitionGroup>
</template>
