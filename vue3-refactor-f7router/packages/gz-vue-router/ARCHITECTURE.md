...
| `composables.ts` | 面向组件的守卫注册入口：`onBeforeRouteLeave()`/`onBeforeRouteUpdate()`，找到当前组件所属的 `entryId`（`inject(ENTRY_ID_KEY)`），登记到 `router.ts` 内部的 `leaveGuardsMap`/`updateGuardsMap`，组件卸载时自动注销。 | `onBeforeRouteLeave`、`onBeforeRouteUpdate` |
...
| `components/GZRouterView.vue` | 双重身份：**根视图**——用在 `App.vue` 里，读 `router.stack`，`<TransitionGroup>` 做前进/后退滑动过渡；**嵌套视图**——用在某个布局组件自己的模板里，靠 `inject` 到的 `DEPTH_KEY` 判断自己是嵌套的，直接渲染路由记录链里对应深度的组件，不做任何动画。 | 默认导出，`index.ts` 里重导出为 `GZRouterView` |
...
① modalStack.length > 0
   ...
   guards  = leaving 的 leave 守卫 + beforeEach
...
② stack.length > 1（没有弹层挡着，页面栈本身有上一页）
   leaving = 当前 top
   guards  = leaving 的 leave 守卫 + beforeEach
   withGuards(..., to=toLocation(stack 里倒数第二条), mutate):
...
③ canGoBack.value 为真（内存里没有上一页，但浏览器 session history 里确实有——多半是刷新丢的）
   凑不出精确的 to（浏览器不暴露上一条历史对应的路由），先用 UNKNOWN_LOCATION 占位跑 leave 守卫 + beforeEach
...
④ 以上都不成立：这个 tab 里压根没有更早的历史
   ...
   guards = leaving 的 leave 守卫 + beforeEach（这次能算出精确的 to，所以不需要在 popstate 里补）
   withGuards(..., mutate):
...
window.addEventListener('popstate', (event) => {
  ...
  ① modalStack 里能找到 state.seq（在 modalStack[modalIndex]）
     且 modalIndex < modalStack.length - 1（说明是往栈里更靠前的位置退，必然是"后退"）：
       guards = 当前最上层弹层的 leave 守卫 + beforeEach
       withGuards(..., to=modalStack[modalIndex] 对应的路由, mutate):
  ...
  ② stack 里能找到 state.seq（在 stack[pageIndex]）
     且（pageIndex 不是当前 top，或者当前挂着弹层）：
       guards = （只有没挂弹层时才算）当前 top 的 leave 守卫 + beforeEach
       withGuards(..., to=stack[pageIndex] 对应的路由, mutate):
         direction.value = 'backward'
  ...
  ③ 两个栈都找不到 state.seq：多半是刷新/直接深链接丢了内存历史
     ...
     matched 是普通路由:
       guards = 当前 top 的 leave 守卫 + beforeEach + collectChainGuards(matched)
       withGuards(..., mutate):
...
## 九、执行顺序：`onBeforeRouteLeave` / `onBeforeRouteUpdate` 的注册与触发

```
组件 setup() 执行期间：
  onBeforeRouteLeave(guard)                          // composables.ts
    ...
    router._registerLeaveGuard(entryId, guard)         // 塞进 router.ts 内部的 leaveGuardsMap
    onUnmounted(() => router._unregisterLeaveGuard(...))  // 组件卸载时自动清理

导航发生时（push/replace/back/popstate 任一分支）：
  guards 数组拼接时会读 leaveGuardsMap.get(entryId) 或 updateGuardsMap.get(entryId)
...
## 十、执行顺序：嵌套路由渲染（`<GZRouterView>` 的双重身份）
